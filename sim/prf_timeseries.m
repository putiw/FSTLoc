...
vec = stimcontrast;
vec(stimcontrast>0) = 1;
startIndices = [];
endIndices = [];
midPoints = [];

% Find start and end indices of each block
for i = 2:length(vec)
    if vec(i) == 1 && vec(i-1) == 0
        startIndices(end+1) = i;
    elseif vec(i) == 0 && vec(i-1) == 1
        endIndices(end+1) = i-1;
    end
end

% Calculate middle points
for i = 1:length(startIndices)
    midPoints(end+1) = floor((startIndices(i) + endIndices(i)) / 2);
end
sd = 4;
...
for iRoi = 1:numel(whichROI)
    ...
    % Calculate the spread - in this example, we use standard deviation as the spread
    % This example assumes a simplistic approach; you may need a more sophisticated method

    numGap = 10;
    myrange = (max(X)-min(X))/numGap;
    spread = zeros(numGap,1);
    fitmean = zeros(numGap,1);
    for ii = 1:numGap
        spread(ii) = std(Y((X>=min(X)+(ii-1)*myrange)&X<(min(X)+ii*myrange)))./sqrt(sum((X>=min(X)+(ii-1)*myrange)&X<(min(X)+ii*myrange)));
        %spread(ii) = std(Y((X>=min(X)+(ii-1)*myrange)&X<(min(X)+ii*myrange))); %./sqrt(sum((X>=min(X)+(ii-1)*myrange)&X<(min(X)+ii*myrange)));

        fitmean(ii) = mean(Y((X>=min(X)+(ii-1)*myrange)&X<(min(X)+ii*myrange)));
    end
    x0plot = min(X):myrange:max(X);
    x0plot(end) = [];

    % Create upper and lower bounds for the spread around the fit line
    upperBound = fitmean + spread;
    lowerBound = fitmean - spread;

    % Creating the patch
    x2 = [x0plot, fliplr(x0plot)]'; % This should be a vertical concatenation to create a single array
    inBetween = [lowerBound; flipud(upperBound)];
    fill(x2, inBetween, [0.5 0.5 0.5], 'LineStyle', 'none','FaceAlpha',0.5); % Create the patch

    plot(x0plot, fitmean, 'k-', 'LineWidth', 2); % Overlay the fit line
