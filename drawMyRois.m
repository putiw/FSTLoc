...
%   end
  ...
  if ~isequal(viewmode,oldviewmode)
    ...
    for zz=1:size(alldata,1)
     plotVal = [reshape(alldata{zz,1},numel(alldata{zz,1}),1);reshape(alldata{zz,2},...
          numel(alldata{zz,2}),1)];
      % [~,Lookup{zz},~,himgs{zz}] = cvnlookup(subjid,viewmode,...
      %     plotVal,crngs{zz},cmaps{zz},[],[],2,{'overlayalpha',plotVal > minVal{zz},'roiname',{'V1@Glasser2016','MT@Glasser2016','MST@Glasser2016','FST@Glasser2016','V7@Glasser2016','V3A@Glasser2016','FEF@Glasser2016'},'roicolor','k','drawroinames',0,'roiwidth',0.1,'fontsize',20,'surfshading',false});
      [~,Lookup{zz},~,himgs{zz}] = cvnlookup(subjid,viewmode,...
          plotVal,crngs{zz},cmaps{zz},[],[],2,{'overlayalpha',plotVal > minVal{zz}});
...
%    pause(1);  % some delay is necessary in order to make sure the main ROI figure is created
  ...
  case 15  % Save
    ...
    while 1
      [savefilename,savepathname] = uiputfile('lh.rois.mgz','Save ROI labels');
      ...
      savepathname = ['/Users/pw1246/Desktop/MRI/FSTLoc/derivatives/rois/' sub];
